...
#define _FOLD_ "C:\\Users\\ATIV\\Desktop\\aes파형\\"
...
#define startpoint 5000
#define endpoint 15000
...
void CPA() {
	...
	sprintf_s(buf, 256 * sizeof(char), "%s%s", _FOLD_, PlaintextFN);		//평문 정보 할당
	if ((err = fopen_s(&rfp, buf, "r")))		//rfp에 평문 정보 할당, 리드로 오픈
	{
		printf("File Open Error2!!\n");
		exit(1);
	}		//파일 오픈 에러 및 평문 할당

	plaintext = (unsigned char**)calloc(TraceNum, sizeof(unsigned char*));		//plaintext 공간 tracenum만큼 0으로 초기화
	for (i = 0; i < TraceNum; i++) {
		plaintext[i] = (unsigned char*)calloc(16, sizeof(unsigned char));
	}		//표라고 생각하고 평문 부분 calloc으로 초기화
	...
	for (i = 0; i < TraceNum; i++) {
		plaintext[i] = (unsigned char*)malloc(16, sizeof(unsigned char));
	}
